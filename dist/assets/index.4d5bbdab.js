...
const createUser = (email, password, displayName, date) => {
...
const likes = [];
const createPost = (post2) => {
  return Xl(Ja(db, "post"), {
    post: post2,
...
const queryInstruction = () => ol(Ja(db, "post"), dl("createDate", "desc"));
const deletePost = (id2) => Zl(Za(db, "post", id2));
const getPost = (id2) => Kl(Za(db, "post", id2));
const updatePost = (id2, editedPost) => Yl(Za(db, "post", id2), editedPost);
const toLike = (id2, uid) => Yl(Za(db, "post", id2), {
  likes: _f(uid)
});
const toDislike = (id2, uid) => Yl(Za(db, "post", id2), {
  likes: wf(uid)
});
const home = () => {
  ...
  formLogin.addEventListener("submit", (e) => {
    e.preventDefault();
    logIn(inpEmail.value, inpPassword.value).then((credentials) => {
...
const register = () => {
  ...
  formRegister.addEventListener("submit", (e) => {
    e.preventDefault();
    createUser(inpEmail.value, inpPassword.value, inpName.value, inpDate.value).then(() => {
      onNavigate("/feed");
    }).catch((error) => {
      const errorCode = error.code;
...
const feed = () => {
  ...
  if (user !== null) {
    user.providerData.forEach(async (profile) => {
      ...
      imgUser.src = photo;
      const name2 = profile.displayName;
      userName.textContent = name2;
      if (photo === null) {
        imgUser.src = "./img/user.png";
      }
      const docRef = Za(db, "users", user.uid);
      const docSnap = await Kl(docRef);
      if (docSnap.exists()) {
      ...
      btnsLikes.forEach((btn) => {
        btn.addEventListener("click", async () => {
          ...
          if (getIdPost === posts.id) {
            const document2 = await getPost(posts.id);
            const post2 = document2.data();
            if (post2.likes.includes(user.uid)) {
      ...
      btnsEdit.forEach((btn) => {
        ...
        if (getIdPost === posts.id) {
          ...
          btn.addEventListener("click", async () => {
            ...
            const document2 = await getPost(posts.id);
            const post2 = document2.data();
            modalToEdit.querySelector(".userImgEdit").src = post2.photo;
            modalToEdit.querySelector(".nameUserEdit").textContent = post2.ownerPost;
            modalToEdit.querySelector(".textAreaEdit").value = post2.post;
